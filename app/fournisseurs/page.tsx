...
const emptyForm = { nom: "", contact: "", telephone: "", ville: "" };
...
export default function FournisseursPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`${API}/fournisseurs${q}`).then((r) => r.json()).then(setFournisseurs);
  }, [filter]);
  ...
  const openEdit = (f: Fournisseur) => {
    setEditTarget(f);
    setForm({ nom: f.nom, contact: f.contact || "", telephone: f.telephone || "", ville: f.ville || "" });
    setError("");
    setModal(true);
  };
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      const url = editTarget ? `${API}/fournisseurs/${editTarget.id}` : `${API}/fournisseurs`;
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white pb-20 sm:pb-6">
      ...
      <div className="px-6 py-4 flex flex-col gap-3">
        ...
        {fournisseurs.map((f) => (
          <div key={f.id} className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex items-start justify-between gap-3">
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 px-4 pb-4 sm:pb-0">
          <div className="bg-slate-800 rounded-2xl w-full max-w-md p-6 flex flex-col gap-4">
            ...
            <form onSubmit={save} className="flex flex-col gap-3">
              ...
              <input placeholder="Contact (personne)" value={form.contact} onChange={(e) => set("contact", e.target.value)}
                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500" />
