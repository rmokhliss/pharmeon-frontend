...
export default function StockForm({ type }: Props) {
  ...
  const [form, setForm] = useState({ productId: "", quantite: "", note: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ productId: "", quantite: "", note: "" });
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input type="text" value={form.note} onChange={(e) => set("note", e.target.value)}
              placeholder={isEntree ? "Ex: Livraison fournisseur NUXE" : "Ex: Pharmacie Centrale Casablanca"}
